...
#if defined(NDEBUG)
# error "Blackchain cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x07742523b571253fab8df0ba6cc306e636effe7a44e6a687aa25b3347a7f3ebe");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Blackchain: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Blackchain Signed Message:\n";
...
{
    // Blackchain: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 120000); // Blackchain: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 24 * 60 * 60; // Blackchain: 3.5 days
static const int64 nTargetSpacing = 3 * 60; // Blackchain: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Blackchain: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Blackchain: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9596374d09b6ab6a48f1ebf24885888af3638742efd040a42dc66a1ba9c54f92");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "May 3, 2020 AD, our coin was born. It is amazing.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e49b0178230bee503a6c1291797c99d16d15e8327877e7b00e323ca1580fb9d7b231764d3b4e08ce95b129fa03c2ec92c23ea90d709d68c8526e5d9e09c7ca1e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1588518000;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084576899;

        if (fTestNet)
        {
            block.nTime    = 1588517400;
            block.nNonce   = 387167884;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x51930ee9f53cfd57f6c289cc888d4f6f6b053e49ab8a969acd32c8dbd8066c76"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Blackchain: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BlackchainMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BlackchainMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BlackchainMiner(CWallet *pwallet)
{
    printf("BlackchainMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("blackchain-miner");
    ...
    try { loop {
        ...
        printf("Running BlackchainMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BlackchainMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BlackchainMiner, pwallet));
}
